...
|-------|------|
|||
|1 a|$\triangle RQS \cong \triangle CBA$ by SAS congruence.|
|1 b|Because $\angle Y = 55^\circ $, $\triangle TUV \cong \triangle WYZ$ by by SAS similarity.|
|1 c|$\angle DCE \cong \angle BCA$ because they are vertical angles; so $\triangle DCE \cong \triangle BCA$ by AAS congruence.|
|1 d|$\triangle FGE \cong \triangle EGH$ by AA Similarity; note that they are not congruent by AAS because side EG, which is the shared side, is not opposite corresponding angles in the triangles—i.e. it is opposite the <span>$20^\circ $</span> angle in one and opposite the <span>$30^\circ $</span> angle in the other.|
